...
function fixture(state = { studentId: 'S1001', plan: [] }) {
  ...
  for (const file of ['course-data.js', 'data.js', 'app.js', 'schedule-tools.js']) {
...
test('context omits personal details, grades, and notes but includes current planning data', async () => {
  ...
  assert.ok(result.completedCourseCodes.includes('CPSC221'));
...
test('search normalizes spaced course codes and reports missing prerequisites when requested', async () => {
  ...
  assert.equal((await app.call('search_course_sections', { query: 'cpsc 320' })).count, 0);
  const result = await app.call('search_course_sections', { query: 'cpsc 320', eligibleOnly: false });
...
test('preview rejects unknown IDs, conflicts, duplicates, wrong term/campus and completed courses', async () => {
  const app = fixture();
  for (const sectionIds of [['unknown'], ['S27-CPSC340-A'], ['F26-CPSC110-A'],
    ['F26-CPSC310-A', 'F26-CPSC310-B'], ['F26-CPSC317-A']]) {
...
test('credit targets use actual mixed course credits, honor saved loads and never underfill', async () => {
  ...
  assert.equal(result.suggestions[0].totalCredits, 13); // Current CPSC choices are 3+4+3+3; no exact 12.
...
test('hard time/day constraints include every meeting; impossible requests do not relax them', async () => {
  ...
  assert.equal((await app.call('suggest_schedules', { courseCodes: ['CPSC340'], courseCount: 1 })).found, false);
});

test('search explores alternate sections to accommodate mandatory courses', async () => {
  const app = fixture();
  // Force CPSC320 to collide with CPSC310-A. CPSC310-B remains a feasible alternative.
  vm.runInContext(`Object.assign(sections.find(s => s.id === 'F26-CPSC320-A'), { days: ['Mon', 'Wed'], startTime: '11:00', endTime: '12:15' })`, app.context);
  const result = await app.call('suggest_schedules', { courseCount: 2, courseCodes: ['cpsc 320', 'CPSC310'] });
  assert.equal(result.found, true);
  assert.ok(result.suggestions[0].sectionIds.includes('F26-CPSC310-B'));
});

test('asynchronous sections are not excluded by time constraints', async () => {
  const app = fixture();
  vm.runInContext(`Object.assign(sections.find(s => s.id === 'F26-CPSC310-B'), { days: [], startTime: '', endTime: '' })`, app.context);
  const result = await app.call('suggest_schedules', { courseCount: 1, courseCodes: ['CPSC310'], earliestStart: '23:00', format: 'online' });
  assert.equal(result.found, true);
  assert.deepEqual(result.suggestions[0].sectionIds, ['F26-CPSC310-B']);
});
...
test('apply is atomic, rejects stale edits, and refreshes the UI only after saving', async () => {
  ...
  assert.deepEqual(app.state().plan, []);
  assert.equal((await app.call('apply_schedule', { sectionIds: ['F26-CPSC320-A'], expectedPlan: [] })).ok, true);
  assert.deepEqual(app.events, ['schedule-plan-changed']);
  assert.match((await app.call('apply_schedule', { sectionIds: ['F26-CPSC310-A'], expectedPlan: [] })).error, /plan changed/);
  assert.deepEqual(app.state().plan, ['F26-CPSC320-A']);
});

test('add is idempotent and rejects duplicate course sections; removal repairs invalid plans', async () => {
  const app = fixture();
  assert.equal((await app.call('apply_schedule', { addSectionId: 'F26-CPSC310-A', expectedPlan: app.state().plan || [] })).ok, true);
  assert.equal((await app.call('apply_schedule', { addSectionId: 'F26-CPSC310-A', expectedPlan: app.state().plan || [] })).ok, true);
  assert.equal((await app.call('apply_schedule', { addSectionId: 'F26-CPSC310-B', expectedPlan: app.state().plan || [] })).ok, false);
  assert.equal(app.state().plan.length, 1);
  const broken = fixture({ studentId: 'S1001', plan: ['unknown', 'F26-CPSC310-A', 'F26-CPSC310-B'] });
  assert.equal((await broken.call('apply_schedule', { removeSectionId: 'unknown', expectedPlan: broken.state().plan || [] })).ok, true);
  assert.equal((await broken.call('apply_schedule', { removeSectionId: 'F26-CPSC310-B', expectedPlan: broken.state().plan || [] })).valid, true);
});
...
test('cancellation and localStorage failure do not report successful mutations', async () => {
  ...
  assert.equal((await tool.execute({ addSectionId: 'F26-CPSC320-A', expectedPlan: [] }, { signal: { aborted: true } })).ok, false);
  vm.runInContext('localStorage.setItem = () => { throw new Error("Storage blocked"); }', app.context);
  assert.match((await app.call('apply_schedule', { addSectionId: 'F26-CPSC320-A', expectedPlan: app.state().plan || [] })).error, /Storage blocked/);
  assert.deepEqual(app.state().plan, []);
  assert.equal(app.events.length, 0);
});
...
test('unavailable times exclude matching meetings everywhere without removing existing courses', async () => {
  const app = fixture({ studentId: 'S1001', plan: ['F26-CPSC310-B'] });
  ...
  assert.deepEqual(app.state().plan, ['F26-CPSC310-B']);
  assert.ok((await app.call('search_course_sections', { query: 'CPSC310' })).sections.every(s => s.id !== 'F26-CPSC310-B'));
  assert.equal((await app.call('preview_schedule', { sectionIds: ['F26-CPSC310-B'] })).valid, false);
  const drafts = await app.call('suggest_schedules', { courseCount: 1, courseCodes: ['CPSC310'] });
  assert.ok(drafts.suggestions.every(draft => !draft.sectionIds.includes('F26-CPSC310-B')));
  assert.equal((await app.call('apply_schedule', { sectionIds: ['F26-CPSC310-B'], expectedPlan: ['F26-CPSC310-B'] })).ok, false);
});
...
test('availability accepts adjacent times, validates overnight blocks, and rejects stale updates', async () => {
  ...
  assert.equal((await app.call('preview_schedule', { sectionIds: ['F26-CPSC310-A', 'F26-CPSC304-A'] })).valid, true);
...
test('comparison calculates exact weekly gaps and separates online meetings from campus days', async () => {
  ...
  const result = await app.call('preview_schedule', { schedules: [
    { label: 'Two campus days', sectionIds: ['F26-CPSC310-A', 'F26-CPSC304-A'] },
    { label: 'Four class days', sectionIds: ['F26-CPSC310-B', 'F26-CPSC304-A'] }
  ] });
...
test('comparison flags different loads, ignores empty/invalid options for ranking, and handles async classes', async () => {
  const app = fixture();
  vm.runInContext(`Object.assign(sections.find(s => s.id === 'F26-CPSC455-A'), { days: [], startTime: '', endTime: '' })`, app.context);
  const result = await app.call('preview_schedule', { schedules: [
    { sectionIds: [] }, { sectionIds: ['unknown'] }, { sectionIds: ['F26-CPSC455-A'] }, { sectionIds: ['F26-CPSC310-A'] }
  ], rankBy: 'campusDays' });
...
test('overlapping classes do not double-count minutes or create negative gaps', async () => {
  const app = fixture();
  vm.runInContext(`Object.assign(sections.find(s => s.id === 'F26-CPSC304-A'), { startTime: '11:30', endTime: '12:45' })`, app.context);
  const plan = await app.call('preview_schedule', { sectionIds: ['F26-CPSC310-A', 'F26-CPSC304-A'] });
...
test('section explanations expose the real score and both preference and degree trade-offs', async () => {
  const app = fixture({ studentId: 'S1001', plan: ['F26-CPSC304-A', 'F26-CPSC455-A'],
    preferences: { priority: 'electives', interests: ['data'], days: ['Tue'], timeOfDay: 'morning', format: 'online' } });
  const result = (await app.call('preview_schedule', { explainSectionId: 'F26-CPSC304-A' })).explanation;
  ...
  const extra = (await app.call('preview_schedule', { explainSectionId: 'F26-CPSC340-A' })).explanation;
...
test('swap previews preserve other courses and same-time searches respect every meeting window', async () => {
  const app = fixture({ studentId: 'S1001', plan: ['F26-CPSC310-A', 'F26-CPSC304-A'] });
  const options = await app.call('suggest_schedules', { replaceSectionId: 'F26-CPSC304-A' });
  assert.ok(options.alternatives.some(option => option.replacementSectionId === 'F26-CPSC455-A'));
  assert.ok(options.alternatives.every(option => option.sectionIds[0] === 'F26-CPSC310-A'));
  assert.equal((await app.call('suggest_schedules', { replaceSectionId: 'F26-CPSC304-A', sameTimeOnly: true })).alternatives.length, 0);
  vm.runInContext(`Object.assign(sections.find(s => s.id === 'F26-CPSC455-A'), { startTime: '15:15', endTime: '16:00' })`, app.context);
  assert.equal((await app.call('suggest_schedules', { replaceSectionId: 'F26-CPSC304-A', sameTimeOnly: true })).alternatives[0].replacementSectionId, 'F26-CPSC455-A');
  assert.deepEqual(app.state().plan, ['F26-CPSC310-A', 'F26-CPSC304-A']);
});

test('swap is atomic, validates before writing, and creates a single undo step', async () => {
  const before = ['F26-CPSC320-A', 'F26-CPSC310-A'];
  const app = fixture({ studentId: 'S1001', plan: before });
  assert.equal((await app.call('apply_schedule', { swap: { sectionId: 'F26-CPSC310-A', replacementSectionId: 'unknown' }, expectedPlan: before })).ok, false);
  assert.equal(app.state().planHistory, undefined);
  assert.equal((await app.call('apply_schedule', { swap: { sectionId: 'F26-CPSC310-A', replacementSectionId: 'F26-CPSC310-B' }, expectedPlan: before })).ok, true);
  assert.deepEqual(app.state().plan, ['F26-CPSC320-A', 'F26-CPSC310-B']);
  assert.equal(app.state().planHistory.length, 1);
  assert.equal((await app.call('apply_schedule', { swap: { sectionId: 'F26-CPSC310-A', replacementSectionId: 'F26-CPSC304-A' }, expectedPlan: before })).ok, false);
...
test('undo respects later manual edits, persists across reload, and rejects stale callers', async () => {
  const app = fixture();
  await app.call('apply_schedule', { addSectionId: 'F26-CPSC320-A', expectedPlan: app.state().plan || [] });
  vm.runInContext(`setState({ plan: ['F26-CPSC320-A', 'F26-CPSC310-A'] })`, app.context);
  ...
  assert.equal((await restoredSession.call('apply_schedule', { undo: true, expectedPlan: ['F26-CPSC320-A'] })).ok, false);
  assert.equal((await restoredSession.call('apply_schedule', { undo: true, expectedPlan: restoredSession.state().plan })).ok, true);
  assert.deepEqual(restoredSession.state().plan, ['F26-CPSC320-A']);
  assert.equal((await restoredSession.call('apply_schedule', { undo: true, expectedPlan: restoredSession.state().plan })).ok, true);
...
test('undo never restores a newly blocked plan or crosses student/term boundaries', async () => {
  const app = fixture();
  await app.call('apply_schedule', { addSectionId: 'F26-CPSC310-A', expectedPlan: app.state().plan || [] });
  await app.call('apply_schedule', { removeSectionId: 'F26-CPSC310-A', expectedPlan: app.state().plan || [] });
...
test('availability overrides constrain drafts, search, comparisons and explanations without saving', async () => {
  ...
  const drafts = await app.call('suggest_schedules', { courseCount: 1, courseCodes: ['CPSC310'], unavailableTimes });
  assert.equal(drafts.found, true);
  assert.deepEqual(drafts.suggestions[0].sectionIds, ['F26-CPSC310-A']);
  assert.deepEqual(drafts.unavailableTimes, unavailableTimes);
  assert.deepEqual(drafts.expectedUnavailableTimes, []);
  const search = await app.call('search_course_sections', { query: 'CPSC310', unavailableTimes });
  assert.deepEqual(search.sections.map(section => section.id), ['F26-CPSC310-A']);
  const preview = await app.call('preview_schedule', { explainSectionId: 'F26-CPSC310-B', unavailableTimes });
  ...
  const comparison = await app.call('preview_schedule', { unavailableTimes, schedules: [
    { sectionIds: ['F26-CPSC310-B'] }, { sectionIds: ['F26-CPSC310-A'] }
  ] });
...
test('omitting availability uses saved blocks, while an empty override ignores them without clearing them', async () => {
  ...
  const app = fixture({ studentId: 'S1001', plan: ['F26-CPSC310-B'], unavailableTimes });
  ...
  assert.equal((await app.call('suggest_schedules', { courseCount: 1, courseCodes: ['CPSC310'], format: 'online' })).found, false);
  const result = await app.call('suggest_schedules', { courseCount: 1, courseCodes: ['CPSC310'], format: 'online', unavailableTimes: [] });
  assert.equal(result.found, true);
...
test('swap suggestions honor optional availability and meeting filters', async () => {
  const app = fixture({ studentId: 'S1001', plan: ['F26-CPSC310-A'] });
  const input = { replaceSectionId: 'F26-CPSC310-A', courseCode: 'cpsc 310' };
  assert.equal((await app.call('suggest_schedules', input)).alternatives[0].replacementSectionId, 'F26-CPSC310-B');
...
test('consolidated tools reject ambiguous and orphaned options before changing state', async () => {
  ...
  const cases = {
    suggest_schedules: [
      { replaceSectionId: 'F26-CPSC310-A', courseCount: 1 },
      { replaceSectionId: 'F26-CPSC310-A', targetCredits: 12 },
      { replaceSectionId: 'F26-CPSC310-A', courseCodes: [] },
      { courseCode: 'CPSC310' }, { sameTimeOnly: false }
    ],
    preview_schedule: [
      { schedules: [], sectionIds: [] }, { schedules: [], explainSectionId: 'F26-CPSC310-A' },
      { rankBy: 'campusDays' }, { explainSectionId: 'unknown' },
      { explainSectionId: 'F26-CPSC310-A', sectionIds: ['unknown'] }
    ],
    apply_schedule: [
      {}, { sectionIds: [], addSectionId: 'F26-CPSC310-A', expectedPlan: [] },
      { undo: false, expectedPlan: [] }, { undo: true, unavailableTimes: [], expectedPlan: [], expectedUnavailableTimes: [] },
      { sectionIds: [] }, { addSectionId: 'F26-CPSC310-A' }, { removeSectionId: 'F26-CPSC310-A' },
      { swap: { sectionId: 'F26-CPSC310-A' }, expectedPlan: [] },
      { unavailableTimes: [] }, { expectedUnavailableTimes: [] }, { expectedPlan: [] },
      { addSectionId: 'F26-CPSC310-A', expectedPlan: ['stale'] },
      { removeSectionId: 'F26-CPSC310-A', expectedPlan: ['stale'] }
    ]
  };
...
test('combined plan and availability save validates both snapshots and commits once', async () => {
  const app = fixture();
  const unavailableTimes = [{ day: 'Tue', startTime: '12:00', endTime: '24:00' }];
  const input = { sectionIds: ['F26-CPSC310-A'], expectedPlan: [], unavailableTimes, expectedUnavailableTimes: [] };
  const before = app.state();
  for (const bad of [
    { sectionIds: ['F26-CPSC310-B'] }, { expectedPlan: ['stale'] }, { expectedUnavailableTimes: unavailableTimes }
  ]) {
  ...
  assert.deepEqual(app.state().plan, ['F26-CPSC310-A']);
  assert.deepEqual(app.state().unavailableTimes, unavailableTimes);
  assert.equal(app.state().planHistory.length, 1);
  assert.deepEqual(app.events, ['schedule-plan-changed']);
  assert.equal((await app.call('apply_schedule', { undo: true, expectedPlan: app.state().plan })).ok, true);
...
test('combined saves leave both plan and commitments untouched when storage fails', async () => {
  ...
  const before = app.state();
  vm.runInContext('localStorage.setItem = () => { throw new Error("Storage blocked"); }', app.context);
  const result = await app.call('apply_schedule', { sectionIds: ['F26-CPSC310-A'], expectedPlan: [],
...
test('student-facing controls use the consolidated tools for drafts, explanations, swaps, availability and undo', async () => {
  ...
  await settle();
  handlers['schedule-section-action']({ detail: { action: 'explain', sectionId: 'F26-CPSC304-A' } });
  await settle();
  assert.match(content(get('section-actions')), /Eligible under your current constraints/);
  handlers['schedule-section-action']({ detail: { action: 'swap', sectionId: 'F26-CPSC304-A' } });
  await settle();
  const swap = buttons(get('section-actions')).find(button => content(button).includes('CPSC455'));
  assert.ok(swap);
  await swap.fire('click');
  assert.deepEqual(app.state().plan, ['F26-CPSC310-A', 'F26-CPSC455-A']);
